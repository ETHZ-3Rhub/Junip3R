...
class IConfigRepository(Protocol):
    ...
    # Only setup's preview implementation (SetupConfigRepository) actually supports
    # these - a real project's config is fixed for the session, so ConfigRepository
    # never implements them and nothing on that path ever calls them.
