...
| Module | Verdict | Notes |
|---|---|---|
| `config/data.py` | low value | `LabellerConfig`/`InstanceType`/`MemberSpecs`/`SkeletonSpecs` are now purely descriptive (no `.new_instance()` — that construction logic moved to `data/types/data.py::new_instance`) — covered indirectly via `test_config_parser.py` and the delegates tests below. |
...
| `data/repository/label.py` | **moderate** | `InstanceMapper` (data ↔ runtime instance mapping) + `JuniperLabelRepository`. The mapper is the more interesting unit (member-type dispatch, `data/types/data.py::new_instance` + field copy-back); the repository is a thin wrapper over `LabelSerializer`. **Implemented.** |
| `data/types/abc.py` | low value | Protocols only. |
| `data/types/delegates.py` | **easy, high value** | The actual label geometry (`Keypoint`, `BoundingBox`, `Polygon`, `Polyline`, `Instance`, `Skeleton`). Non-trivial logic: `_normalize_box` (min/max swap), `BoundingBox.__post_init__` deriving 4 corners, `Polygon`/`Polyline`/`Instance.bounds` (min/max aggregation over sub-bounds, `None` when nothing is set), `is_set`. This is exactly the kind of file where a silent regression (e.g. bounds computed pre- instead of post-normalization) would be easy to introduce and hard to notice visually. **Implemented.** |
...
| `legacy/legacy_label_converter.py` | **easy** | Thin glue between `LegacyLabelLoader` and `LabelSerializer`, but derives `bounding_box_type`/`keypoint_names` from `InstanceType.members` (`MemberSpecs`, purely descriptive) — worth one end-to-end test. **Implemented.** |
